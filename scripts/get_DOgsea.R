...
for (i in 2:ncol(cent.mat))
{
  ...
  name=paste(name,"DOgseaRes",sep=".")
  df=cent.mat[,c(1,i)]
  assign(name, TopDOgsea(df,0.2,my_entrez_gene_info))
}
...
for (i in 1:length(centralities))
{
  ...
  mat=acast(data, Description~ct, value.var="qvalue")
  mat[is.na(mat)]=0 #set NA =0
  mat_breaks <- seq(min(mat), max(mat), length.out = 10)
  filename=paste(c,"DOgsea.pdf",sep=".")
  mainTitle=c
  pheatmap::pheatmap(
    mat               = t(mat),
